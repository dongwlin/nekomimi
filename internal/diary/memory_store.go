...
func (s *MemoryStore) Write(ctx context.Context, sessionKey string, entry Entry) (Entry, error) {
	...
	s.entries[key] = append(s.entries[key], normalized)
...
func (s *MemoryStore) List(ctx context.Context, sessionKey string, opts ListOptions) (ListResult, error) {
	...
	notes := s.entries[key]
	if len(notes) == 0 || offset >= len(notes) {
		return ListResult{}, nil
	}

	limit := opts.Limit
	if limit <= 0 {
		limit = len(notes) - offset
	}
	...
	for i := offset; i < end; i++ {
		entries = append(entries, cloneEntry(notes[len(notes)-1-i]))
	}
...
func (s *MemoryStore) nextEntryIDLocked() string {
	s.nextID++
	return "diary_" + strconv.FormatUint(s.nextID, 10)
}
...
func cloneEntry(entry Entry) Entry {
	cloned := entry
	if len(entry.Tags) > 0 {
		cloned.Tags = make([]string, len(entry.Tags))
		copy(cloned.Tags, entry.Tags)
	}
