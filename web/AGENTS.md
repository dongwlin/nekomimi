...
- API requests use generated client wrappers in `src/lib/api/auth.ts` and global config in `src/lib/api/client.ts`.
- 401 responses trigger refresh-once + retry logic; refresh failure clears session and redirects to `/login`.
...
- Generate TS API client (from backend swagger): `pnpm gen:api-client`
- From repo root, use `pnpm -C web <script>` to run the same commands.

## Generated Code
- `src/lib/api/generated/*` is generated by `@hey-api/openapi-ts`; do not edit manually.
- If backend API contract changes, regenerate with `pnpm gen:api-client`.
